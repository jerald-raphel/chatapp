...
// // Socket connection
// const socket = io('http://localhost:1000'); // Adjust the URL if needed
...
//     return () => {
//       socket.disconnect(); // Disconnect socket when the component is unmounted
//       clearTimeout(timer);
//     };
//   }, []);
...
//       // Add the new message to the userMessages state
//       setUserMessages((prevMessages) => [...prevMessages, newMessage]);
//       setMessageInput('');

//       // Emit the new message to other clients using Socket.IO
...
//     setMessageInput('');
//     setUserMessages([]); // Clear old messages when switching user
//   };
...
//                     key={index}
//                     className={`message ${
//                       message.senderName === loggedInUser.name ? 'sent' : 'received'
//                     }`}
//                   >

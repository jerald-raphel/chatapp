...
function ChatPage() {
  ...
  const [desktopChatView, setDesktopChatView] = useState(false);
  ...
  const handleChangeUser = (user) => {
    ...
    if (window.innerWidth <= 480) {
      ...
      setDesktopChatView(true);
    }
  };

  const handleCloseChat = () => {
    setMobileChatView(false);
    setDesktopChatView(false);
  ...
  return (
    <div className="chat-container" style={{ backgroundColor: theme.background, color: theme.text }}>
      ...
      <div className="main-section">
        ...
        {/* Chat Window */}
        {(desktopChatView || mobileChatView) && selectedUser && (
          <section className="chat-window" style={{ backgroundColor: theme.formBackground }}>
            <div className="chat-header">
              {mobileChatView && <span className="back-arrow" onClick={handleCloseChat}>←</span>}
              <div>
                <span className="name">{selectedUser}</span>
                <span className="status">{isUserOnline ? 'Online' : 'Offline'}</span>
              </div>
